...
public class RTPCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (randomLocation == null) {
            player.sendMessage(plugin.getConfigManager().getEssentialsConfig().getString("rtp.no-safe-location", "Could not find a safe location to teleport!"));
            return true;
        }
        ...
        player.sendMessage(plugin.getConfigManager().getEssentialsConfig().getString("rtp.teleporting", "Random teleporting in %delay% seconds...").replace("%delay%", String.valueOf(delay)));

        plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
            player.teleport(finalLocation);
            player.sendMessage(plugin.getConfigManager().getEssentialsConfig().getString("rtp.teleported", "Random teleported!"));
        }, 20L * delay);
