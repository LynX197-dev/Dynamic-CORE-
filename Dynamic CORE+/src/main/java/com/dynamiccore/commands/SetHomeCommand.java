...
public class SetHomeCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (homes.size() >= maxHomes && !homes.containsKey(homeName)) {
            player.sendMessage(plugin.getConfigManager().getEssentialsConfig().getString("sethome.max-homes", "You can only have %max% homes!").replace("%max%", String.valueOf(maxHomes)));
            return true;
        }

        plugin.getPlayerDataManager().setHome(player, homeName, player.getLocation());
        player.sendMessage(plugin.getConfigManager().getEssentialsConfig().getString("sethome.set", "Home '%home%' set!").replace("%home%", homeName));
        return true;
    }
}